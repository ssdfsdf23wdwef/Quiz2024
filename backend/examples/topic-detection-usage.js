...
exports.LessonController = exports.ExampleUsage = void 0;
class ExampleUsage {
    topicDetectionService;
    constructor(topicDetectionService) {
        this.topicDetectionService = topicDetectionService;
    }
    ...
    `;
        ...
        try {
            const newTopics = await this.topicDetectionService.detectExclusiveNewTopics(lessonContext, existingTopics);
    ...
    `;
        try {
            const newTopics = await this.topicDetectionService.detectExclusiveNewTopics(lessonContext, []);
...
class LessonController {
    ...
    async addNewLessonContent(lessonId, newContent) {
        try {
            const existingTopics = await this.getExistingTopicsForLesson(lessonId);
            const newTopics = await this.topicDetectionService.detectExclusiveNewTopics(newContent, existingTopics);
    ...
    async getExistingTopicsForLesson(lessonId) {
        return [];
    }
    async addTopicsToLesson(lessonId, topics) {
        console.log(`Adding topics to lesson ${lessonId}:`, topics);
    }
}
exports.LessonController = LessonController;
//# sourceMappingURL=topic-detection-usage.js.map